...
/*   Updated: 2025/12/01 21:07:23 by shitakah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static t_stash	stash[GNL_MAX_FD];
	t_stash			*st;
	char			*line;
	size_t			consumed;
	...
	if (!st->buf || consumed >= st->len)
		return (free_stash(st), line);
	ft_memmove_gnl(st->buf, st->buf + consumed, st->len - consumed);
	st->len -= consumed;
	st->buf[st->len] = '\0';
	return (line);
}
